function [out, Cellula1] = metrics_cellpose(settings)
close all;
...
% Call Cellpose for the initial frame
%MaskIni = cellpose(FrameVideo, 'Threshold', Thrs, 'ModelType', 'cyto2');
%MMM = MaskIni; % Use the output masks from Cellpose for initial segmentation
...
s = regionprops(bwlabel(MMM),'MajorAxisLength');
MaxAx = round(cat(1,s.MajorAxisLength));

RadiusCells = averageCellDiameter;
...
for r1=FrameInit:FrameEnd
    ...
    Cellula1(:,:,r1-FrameInit+1)=FrameVideo(Traj1(r1-FrameInit+1,1)-RadiusCells(1):Traj1(r1-FrameInit+1,1)+RadiusCells(1),Traj1(r1-FrameInit+1,2)-RadiusCells(1):Traj1(r1-FrameInit+1,2)+RadiusCells(1));
     imagesc(Cellula1(:,:,r1-FrameInit+1));colormap gray;axis image;axis off;
     hold on;
     drawnow
end
time = toc;
%time = Tempo/NumSeconds;
